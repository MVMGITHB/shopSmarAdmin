...
const Users = () => {
  ...
  const fetchData = async () => {
    try {
      const res = await axios.get(baseurl + "/api/user/getAllAdmin");
  ...
  const handleStatusToggle = async (record) => {
    try {
      const response = await axios.patch(
        `${baseurl}/api/use/toggled/${record?._id}`
      );
  ...
  const handleDelete = async(record)=>{
    try {
         const response = await axios.delete(`${baseurl}/api/user/deleteUser/${record}`)
         if (response) {
            message.success("Status updated succesfully");
            fetchData();
        }
...
// const b = String(formatted)
  ...
  const handlePut = async (values) => {
    ...
    try {
      const response = await axios.patch(
        `${baseurl}/api/user/updateUser/${editingUser?._id}`,
        postData
      );
      console.log(response.data);
